...
InsertStatement *parse_insert(Parser *parser) {
    ...
    if (!expect(parser, TOKEN_KEYWORD_INSERT, "INSERT")) goto error;
    if (!expect(parser, TOKEN_KEYWORD_INTO, "INTO")) goto error;
    Token *table = expect(parser, TOKEN_IDENTIFIER, "table name");
    if (!table) goto error;
    if (match(parser, TOKEN_PUNC_OPEN_PAREN)) {
        ...
        if (!expect(parser, TOKEN_PUNC_CLOSE_PAREN, "')'")) goto error;
    }
    if (!expect(parser, TOKEN_KEYWORD_VALUES, "VALUES")) goto error;
    if (!expect(parser, TOKEN_PUNC_OPEN_PAREN, "(")) goto error;

    do {
        Token *val_token = advance(parser);
        if (!val_token) goto error;
    ...
    if (!expect(parser, TOKEN_PUNC_CLOSE_PAREN, "')'")) goto error;
    if (!expect(parser, TOKEN_END_OF_LINE, "';'")) goto error;
...
CreateStatement *parse_create(Parser *parser) {
    ...
    if (!expect(parser, TOKEN_KEYWORD_CREATE, "CREATE")) goto error;
    if (!expect(parser, TOKEN_KEYWORD_TABLE, "TABLE")) goto error;
    ...
    do {
        ...
        if (!col_name) goto error;

        Token *type_token = peek(parser);
        if (!type_token) goto error;
    ...
    error:
        ...
        for (size_t i = 0; i < column_count; i++) free(columns[i].name);
        free(columns);
        return NULL;
}

ControlStatement *parse_begin(Parser *parser) {
    if (!expect(parser, TOKEN_KEYWORD_BEGIN, "BEGIN")) return NULL;
    if (!expect(parser, TOKEN_KEYWORD_TRANSACTION, "TRANSACTION")) return NULL;
    if (!expect(parser, TOKEN_END_OF_LINE, "';'")) return NULL;
...
ControlStatement *parse_commit(Parser *parser) {
    if (!expect(parser, TOKEN_KEYWORD_COMMIT, "COMMIT")) return NULL;
    if (!expect(parser, TOKEN_END_OF_LINE, "';'")) return NULL;
