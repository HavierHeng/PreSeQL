...
typedef enum {
    PSQL_BUSY,  /* Database engine was unable acquire the locks to do its job.
    ...
    PSQL_DONE,  /* VM engine has finished execution.
                Do not call psql_step() again before psql_reset() is used to reset the engine state */

    PSQL_ROW,  /* VM engine detects that there is a return value in its register, 
                Return the value by calling the appropriate psql_column_*() function */

    PSQL_ERROR,  /* Run-time error, e.g constraint violation has occured
                More information can be obtained by calling psql_errmsg()
                */

    PSQL_MISUSE  /* VM engine was not reset 
